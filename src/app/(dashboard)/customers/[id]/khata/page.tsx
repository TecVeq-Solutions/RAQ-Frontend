...
import { LedgerStatement, Customer } from '@/types/ledger';
import { useParams, useRouter } from 'next/navigation';
import { FileText, ArrowLeft, Calendar, Loader2, Download, Printer } from 'lucide-react';
...
export default function CustomerKhataPage() {
  ...
  const fetchStatement = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClient.get(`/customers/${customerId}/ledger`, { params: queryParams });
      if (res.data?.data) {
        setStatement(res.data.data);
      }
    } catch (err) {
      console.error('Failed to load customer Khata statement', err);
    } finally {
      setLoading(false);
    }
  ...
  const customer: Customer | undefined = statement?.customer;
    ...
    <div className="space-y-6 animate-fadeIn print:space-y-4 print:p-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#0F172A]">Customer Ledger Statement</h1>
            <p className="text-sm text-slate-500">
              Complete historical ledger transaction tracking and client Khata summary
            </p>
          </div>
        </div>
      ...
      {customer && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-6 print:border-none print:shadow-none print:p-0 print:grid-cols-2">
          <div>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Client Name</div>
            <div className="text-base font-extrabold text-[#0F172A] mt-1">{customer.name}</div>
            <div className="text-xs text-slate-400 mt-0.5">{customer.email || 'No email registered'}</div>
          </div>

          <div>
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Contact Number</div>
            <div className="text-sm font-semibold text-slate-700 mt-1">{customer.phone || '-'}</div>
          ...
          <div>
            ...
            <div className="text-base font-bold text-slate-600 mt-1">
              Rs. {Number(customer.opening_balance).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
          </div>

          <div className="md:text-right print:text-left">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Receivable</div>
            <div className="text-lg font-black text-[#16A34A] mt-1">
              Rs. {Number(customer.current_balance).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
          </div>
        </div>
      )}
      ...
      {/* Date Filter Toolbar */}
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-3 print:hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-600 w-full sm:w-auto">
            ...
            <input
              ...
              className="flex-1 sm:flex-none px-3 py-2 rounded-lg border border-slate-200 text-slate-700 focus:outline-none focus:ring-1 focus:ring-[#16A34A] min-h-[38px]"
            />
          </div>
          ...
          <table className="w-full text-left text-sm text-slate-600 print:text-xs">
            <thead className="bg-slate-50 text-xs uppercase font-semibold text-slate-500 border-b border-slate-100 print:bg-transparent">
              <tr>
                ...
                <th className="px-5 py-3 text-right">Debit (Receivable +)</th>
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {loading ? (
                ...
                <>
                  {/* Carry Forward Row (Opening cumulative balance for filtered dates) */}
                  {startDate && (
                          ...
                          {entry.due_date ? (
                            <div className="space-y-1">
                              ...
                              {entry.due_status === 'overdue' && (
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-black bg-rose-50 text-rose-700 border border-rose-200">
                                  Overdue {entry.overdue_days ? `(${entry.overdue_days}d)` : ''}
                                </span>
                              )}
                              {entry.due_status === 'due_today' && (
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-black bg-amber-50 text-amber-800 border border-amber-200">
                                  Due Today
                                </span>
                              )}
                              {entry.due_status === 'due_soon' && (
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold bg-sky-50 text-sky-700 border border-sky-200">
                                  Due Soon
                                </span>
                              )}
