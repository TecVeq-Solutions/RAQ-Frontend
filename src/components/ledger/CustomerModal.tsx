...
export default function CustomerModal({
  ...
  useEffect(() => {
    if (customerToEdit) {
      ...
      setIsActive(customerToEdit.is_active ?? true);
    } else {
      setName('');
      setEmail('');
      setPhone('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (customerToEdit) {
        await apiClient.put(`/customers/${customerToEdit.id}`, payload);
      } else {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2 font-bold text-[#0F172A] text-base">
            ...
            <span>{customerToEdit ? 'Edit Customer Info' : 'Register New Customer'}</span>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            />
            <label htmlFor="customerActive" className="text-xs font-bold text-slate-800 cursor-pointer">
              Active Status
            </label>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            >
              ...
              <span>{customerToEdit ? 'Save Changes' : 'Register Customer'}</span>
            </button>
          </div>
        </form>
      </div>
