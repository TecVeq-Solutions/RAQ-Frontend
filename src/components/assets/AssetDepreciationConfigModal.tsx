      ...
      <div className="relative w-full max-w-2xl my-8 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <div className="font-semibold text-slate-100">Straight-Line Method</div>
                  <div className="text-slate-400 text-[11px] mt-0.5">Equal periodic depreciation expense</div>
                </button>
                ...
                <button
                  ...
                  className={`px-4 py-3 rounded-xl border text-xs font-medium text-left transition-all ${
                    formData.depreciation_method === 'none'
                      ? 'border-slate-600 bg-slate-800/40 text-slate-200 ring-1 ring-slate-600'
                      : 'border-slate-800 bg-slate-950 text-slate-400 hover:text-slate-300'
                  }`}
                >
                  <div className="font-semibold text-slate-100">Non-Depreciable</div>
                  <div className="text-slate-400 text-[11px] mt-0.5">e.g. Land or static capital holding</div>
                </button>
              </div>
            </div>
              ...
              <>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1.5 flex items-center gap-1.5">
                    ...
                    Useful Asset Life (Years) <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.5"
                    ...
                    placeholder="e.g. 5"
                    className={`w-full px-3.5 py-2.5 bg-slate-950 border rounded-xl text-slate-100 text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all ${
                      fieldErrors.useful_life_years ? 'border-rose-500' : 'border-slate-800'
                    }`}
                  />
                  {fieldErrors.useful_life_years && (
                    <p className="mt-1 text-xs text-rose-400">{fieldErrors.useful_life_years}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all"
                  >
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1.5 flex items-center gap-1.5">
                    ...
                    Depreciation Start Date <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="date"
                    value={formData.depreciation_start_date}
                    onChange={(e) => setFormData({ ...formData, depreciation_start_date: e.target.value })}
                    className={`w-full px-3.5 py-2.5 bg-slate-950 border rounded-xl text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all ${
                      fieldErrors.depreciation_start_date ? 'border-rose-500' : 'border-slate-800'
                    }`}
                  />
                  {fieldErrors.depreciation_start_date && (
                    <p className="mt-1 text-xs text-rose-400">{fieldErrors.depreciation_start_date}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className={`w-full px-3.5 py-2.5 bg-slate-950 border rounded-xl text-slate-100 text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all ${
                      fieldErrors.salvage_value ? 'border-rose-500' : 'border-slate-800'
                    }`}
                  />
                  {fieldErrors.salvage_value && (
                    <p className="mt-1 text-xs text-rose-400">{fieldErrors.salvage_value}</p>
                  )}
                </div>
              </>
            )}
          ...
          {formData.depreciation_method === 'straight_line' && usefulYears > 0 && (
            <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/20 text-xs space-y-2.5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-slate-300 pt-1">
                <div>
                  <span className="text-slate-500 block">Depreciable Basis</span>
                  <span className="font-mono font-semibold text-slate-200">
                    PKR {depreciableAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block">Total Periods</span>
                  <span className="font-mono font-semibold text-slate-200">
                    {totalPeriods} {formData.depreciation_frequency === 'monthly' ? 'Months' : 'Years'}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block">Periodic Expense</span>
                  <span className="font-mono font-semibold text-purple-400">
                    PKR {periodicDepreciation.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </span>
                </div>
                <div>
                  <span className="text-slate-500 block">Annual Depreciation</span>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-slate-950 font-semibold text-sm shadow-lg shadow-purple-500/20 disabled:opacity-50 flex items-center gap-2 transition-all"
            >
              {loading ? (
                <>
